package com.nian.myweixinfriend.utils;
...
import com.nian.myweixinfriend.model.UserModel;
...
public class DatasUtil {
    ...
    public static final UserModel curUser = new UserModel("0", "自己", HEADIMG[0]);

    static {
        UserModel user1 = new UserModel("1", "张三", HEADIMG[1]);
        UserModel user2 = new UserModel("2", "李四", HEADIMG[2]);
        UserModel user3 = new UserModel("3", "隔壁老王", HEADIMG[3]);
        UserModel user4 = new UserModel("4", "赵六", HEADIMG[4]);
        UserModel user5 = new UserModel("5", "田七", HEADIMG[5]);
        UserModel user6 = new UserModel("6", "Naoki", HEADIMG[6]);
        UserModel user7 = new UserModel("7", "这个名字是不是很长，哈哈！因为我是用来测试换行的", HEADIMG[7]);
    ...
    public static ArrayList<FriendCircleModel> createCircleDatas() {
        ArrayList<FriendCircleModel> circleDatas = new ArrayList<FriendCircleModel>();
        for (int i = 0; i < 15; i++) {
            FriendCircleModel item = new FriendCircleModel();
    ...
    public static List<FavoriteModel> createFavortItemList() {
        int size = getRandomNum(users.size());
        List<FavoriteModel> items = new ArrayList<FavoriteModel>();
        List<String> history = new ArrayList<String>();
        if (size > 0) {
            for (int i = 0; i < size; i++) {
                FavoriteModel newItem = createFavortItem();
    ...
    public static FavoriteModel createFavortItem() {
    ...
    public static FavoriteModel createCurUserFavortItem() {
        FavoriteModel item = new FavoriteModel();
        item.setId(String.valueOf(favortId++));
    ...
    public static List<CommentModel> createCommentItemList() {
        List<CommentModel> items = new ArrayList<CommentModel>();
     ...
     * @return
     */
    public static CommentModel createPublicComment(String content) {
        CommentModel item = new CommentModel();
        item.setId(String.valueOf(commentId++));
     ...
     * @return
     */
    public static CommentModel createReplyComment(UserModel replyUser, String content) {
        CommentModel item = new CommentModel();
        item.setId(String.valueOf(commentId++));
        item.setContent(content);
        item.setUser(curUser);
